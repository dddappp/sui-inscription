...
slot_number_table_id=$(cat test_sui_publish_package.json | jq -r '.events[] | select(.transactionModule == "slot").parsedJson.id')
...
minted_inscription=$(cat test_minted_inscription.json | jq -r '.objectChanges[] | select(.type == "created") | select(.objectType | test("::inscription::Inscription")).objectId')
echo "minted_inscription: $minted_inscription"

sui client call --package $package_id --module slot_aggregate --function put_up_candidate \
--args $slot_object_id \"$minted_inscription\" \"0x6\" \
